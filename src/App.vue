<template>
  <div ref="app" id="app" v-if="vp">
    <div
      id="viewer"
      :class="{ 'viewer-container': true, ultrawide: vp.settings.ultrawide }"
    >
      <div ref="player" id="player">
        ...
        <div
          v-if="vp.settings.chatOverlay"
          id="comments"
          v-bind:class="commentsClass"
          v-bind:style="commentsStyle"
        >
          <ChatMessage
    ...
    <div id="timeline" ref="timeline" @click="seek">
      <div
        id="timeline-seekbar"
        ref="seekbar"
        v-bind:style="{ width: vp.videoPosition * 100 + '%' }"
      ></div>
    ...
    <div id="controls">
      <div class="option-row">
        <div
          v-if="!vp.automated"
          v-bind:class="{ 'option-group': true, ok: vp.videoLoaded }"
          class="option-group"
        >
          ...
          <div class="option-content">
            ...
            <div v-if="video_source == 'twitch'">
              ...
              <p class="help-text">
                Please proceed through the mature warning before clicking start,
                if it appears.<br />
                This feature requires Twitch API credentials.
              </p>
            </div>
            <hr />
            <button class="button is-submit" @click="submitVideo">
              Submit
            </button>
            <!--
        ...
        <div
          v-if="!vp.automated"
          v-bind:class="{ 'option-group': true, ok: vp.chatLoaded }"
          class="option-group"
        >
              ...
              <div class="control fullwidth">
                <label
                  ><input
                    ...
                    name="chat-input"
                    ref="chat_input"
                    accept="application/json,.chatdump"
                    placeholder="Chat"
                /></label>
              </div>
            </div>
          ...
          <div class="option-content">
            <label>
              <input
                ...
                v-model="vp.settings.twitchClientId"
              />
              Client ID
            </label>
            <label>
              <input
                type="password"
                placeholder="Secret"
                v-model="vp.settings.twitchSecret"
              />
              Secret
            </label>
            <br />
            <span class="is-error">{{
              vp.settings.twitchToken ? "Has token" : "No token"
            }}</span>
            <br />
            <button class="button" @click="saveSettings">Save</button>
            <button class="button" @click="fetchTwitchToken">
              Fetch Twitch token
            </button>
          </div>
        </div>
      ...
      <div class="option-row">
        <div v-if="!vp.automated" class="option-group">
          ...
          <div class="option-content">
            <p class="help-text">
              Offset from the video, if recording started too late. It will be
              set automatically based on how long the chat dump is and the video
        ...
        <div class="option-group">
          ...
          <div class="option-content">
            <p class="help-text">
              The lower the smoother. 16.67 - 60fps, 33.33 - 30fps. Missed ticks
              shouldn't matter, as the parser is dependent on system time.
            </p>
          ...
          <div class="option-content">
            <div>
              <label
                ><input
                  type="checkbox"
                  name="comments-overlay"
                  v-model="vp.settings.chatOverlay"
                />
                Overlay</label
              >
              <label
            ...
            <div>
              ...
              <label
                ><input
                  ...
                  v-model="vp.settings.chatTextAlign"
                />
                Left</label
              >
              <label
            ...
            <label
              ><input
                ...
                min="0"
                max="100"
                v-model="vp.settings.chatBottom"
                style="direction: ltr"
              />
              Bottom</label
            >
                ...
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.chatStroke"
                    />
                    Stroke + shadow</label
                  >
                </td>
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.emotesEnabled"
                    />
                    Emotes</label
                  >
                </td>
                ...
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.timestampsEnabled"
                    />
                    Timestamps</label
                  >
                </td>
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.badgesEnabled"
                    />
                    Badges</label
                  >
                </td>
                ...
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.smallEmotes"
                    />
                    Small emotes</label
                  >
                </td>
                <td>
                  <label
                    ><input
                      type="checkbox"
                      checked="checked"
                      v-model="vp.settings.showVODComments"
                    />
                    VOD comments</label
                  >
                </td>
...
export default defineComponent({
  ...
  methods: {
    submitVideo(event: Event) {
      ...
      this.vp.loadVideo(
        this.video_source,
        this.$refs.video_input as HTMLInputElement
      );
    ...
    submitChat(event: Event) {
      ...
      this.vp.loadChat(
        this.chat_source,
        this.$refs.chat_input as HTMLInputElement
      );
      event.preventDefault();
      return false;
    },
